...
import { COURSE_LEVEL_METADATA, CATEGORY_METADATA, ARTIST_METADATA } from '@/src/utils/constants';
...
const getLevel = () => levelRatings[Math.floor(Math.random() * levelRatings.length)];

const categories = CATEGORY_METADATA.map(({ value }) => value);
const getCategories = (level) => {
  let result = [];
  for (let i = 0; i <= 3; i++) {
    result = [ ...result, categories[Math.floor(Math.random() * categories.length)]];
  }
